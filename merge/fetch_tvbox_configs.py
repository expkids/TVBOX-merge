...
"""
脚本名称：fetch_tvbox_configs.py
功能：从 GitHub 搜索包含特定关键词的代码片段，并保存到本地的 source.txt 文件中。
...
# 🔐 你的 GitHub Personal Access Token（必须要有 repo 权限）
# 请将下面的 'ghp_your_token_here' 替换为你自己的真实 Token
GH_TOKEN = '${GH_TOKEN}'  # ← 请替换为你的真实 Token

# 🧍 你的 GitHub 用户名
GITHUB_USERNAME = 'leexuben'  # ← 请根据你的 GitHub 用户名修改，如 'hxy97'

# 📦 你的目标仓库名（格式：用户名/仓库名）
REPO_NAME = 'leexuben/TVBOX-merge'  # ← 请根据你的仓库名修改，如 'hxy97/tvbox-config-collector'

# 📂 你想要保存的文件路径（相对于仓库根目录）
# 例如：'source.txt' 或 'merge/source.txt'
FILE_PATH = 'source.txt'  # ← 你可以修改为 'merge/source.txt' 或其他路径

# 🔍 搜索关键词（你可以自行增删，比如 tvbox、m3u、源、接口等）
KEYWORDS = ['荐片', '采集', '.spider', 'tvbox', '接口', 'm3u']  # ← 你可以根据需要修改关键词
